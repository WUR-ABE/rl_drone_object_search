...
class ExecutedFlightPath:
    ...
    current_idx: int = 0
    object_map: NDArray[np.bool_] = np.zeros((0, 0), dtype=np.bool_)
...
class Fields2CoverAlgorithm(BaseAlgorithm):
    ...
    def predict(self, *args: Any, **kwargs: Any) -> tuple[NDArray[np.int64], tuple[NDArray[np.float32], ...] | None]:
        ...
        worlds: list[BaseWorld] = self.env.get_attr("world")
    ...
    def get_action(self, flight_path: ExecutedFlightPath, world: BaseWorld, drone: Drone, stopping_criteria: StoppingCriteria) -> ActionSpace | int:
    ...
    def generate_flight_path(self, drone: Drone, world: BaseWorld) -> NDArray[np.uint16]:
    ...
    @staticmethod
    def field_from_world(world: BaseWorld) -> Field:
        ring = f2c.LinearRing()
        ring.addPoint(0, 0)
    ...
    @staticmethod
    def _do_stopping_action(world: BaseWorld, drone: Drone, stopping_criteria: StoppingCriteria) -> int | None:
        action = None
    ...
    @staticmethod
    def _get_closest_landing_spot_path(world: BaseWorld, position: NDArray[np.uint16]) -> NDArray[np.uint16]:
        land_distances = {}
    ...
    def _distance(coordinate_1: NDArray[np.uint16], coordinate_2: NDArray[np.uint16]) -> float:
        return np.linalg.norm(np.array(coordinate_1) - coordinate_2)
