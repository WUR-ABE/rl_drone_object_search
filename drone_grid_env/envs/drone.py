...
from .utils import FlightPath, coordinate_in_list, coordinates_in_size
...
if TYPE_CHECKING:
    ...
    from .world.base import BaseWorld
...
class Drone:
    ...
    def path_length(self) -> float:
        return self._flight_path.get_length()
    ...
    def _get_initial_position(self) -> NDArray[np.uint16]:
        ...
        if self._config.start_mode == StartMode.IN_START_ZONE:
            if len(self._world.config.start_landing_zones) == 0:
                raise RuntimeError("Cannot use 'IN_START_ZONE' initial position because the 'start_landing_zones' key is not defined!")

            random_zone = self._rng.integers(0, len(self._world.config.start_landing_zones))
            return np.array(np.mean(self._world.config.start_landing_zones[random_zone].coordinates, axis=0)).astype(np.uint16)

        raise NotImplementedError(f"Start mode {self._config.start_mode} is not implemented!")

    @classmethod
    def from_config_file(cls, config_file: Path, world: BaseWorld, rng: np.random.Generator) -> Drone:
