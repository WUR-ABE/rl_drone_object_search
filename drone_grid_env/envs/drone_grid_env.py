...
from drone_grid_env import assets, logger
...
from drone_grid_env.envs.utils import count_gt_classified_objects
from drone_grid_env.envs.world import get_world_by_name
...
class DroneGridEnv(Env[ObsType, ActType]):
    ...
    def __init__(
        ...
        self.world = get_world_by_name(self.config["world"]["type"]).from_config_file(self.config_file_path, self.np_random)
        self.drone = Drone.from_config_file(self.config_file_path, self.world, self.np_random)
        self.state_space = StateSpace.from_config_file(self.config_file_path, self.world, self.drone, self.np_random)
    ...
    def config_file_path(self) -> Path:
        if isinstance(self.config_file, str):
            ...
            source = resources.files(assets).joinpath(self.config_file)
    ...
    def _get_info(self) -> dict[str, Any]:
        return {
            ...
            "cwp": (count_gt_classified_objects(self.world, self.drone) / self.world.number_of_objects if self.world.number_of_objects != 0 else 0),
            "cov": np.count_nonzero(self.drone.coverage_map) / (self.world.size[0] * self.world.size[1]),
            "stp": self._steps,
            "pl": self.drone.flight_path.get_length(),
