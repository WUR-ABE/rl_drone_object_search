...
def read_kml(kml_file: Path, dtype: type[DType] = np.uint16) -> tuple[OrderedDict[str, NDArray[DType]], list[dict[str, Any]], dict[str, Any]]:  # type: ignore[assignment]
...
def write_kml_flightpath(
    output_file: Path, data: list[NDArray[np.generic]] | dict[str, list[NDArray[np.generic]]], extended_data: dict[str, Any] = {}
) -> None:
    kml = Kml()
...
def _parse_extended_data(element: ET._Element, namespaces: dict[str, str] | None = None) -> dict[str, Any]:
    extended_data = {}
    if (extended_data_tag := element.find(".//kml:ExtendedData", namespaces=namespaces)) != None:
