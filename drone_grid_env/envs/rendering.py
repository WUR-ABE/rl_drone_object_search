...
class Rendering:
    ...
    def render(
        ...
        if self._drone_element is None:
            source = resources.files(assets).joinpath("drone.png")
            with resources.as_file(source) as drone_image_file:
                self._drone_element = pygame.image.load(drone_image_file).convert_alpha()
