...
class StoppingCriteria:
    ...
    def can_terminate(self, world: BaseWorld, drone: Drone, action: ActionSpace | int) -> bool:
        if self.config.method == "objects":
            return count_gt_classified_objects(world, drone) / world.number_of_objects >= self.config.value if world.number_of_objects > 0 else True
            ...
            if num_found_objects > self._num_found_objects + 1:  # Detect minimal 2 new objects to avoid problems with FP
                ...
                self._step_last_found_object = len(drone.flight_path.positions)

            return (len(drone.flight_path.positions) - self._step_last_found_object) > self.config.value
