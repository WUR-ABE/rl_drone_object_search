...
from drone_grid_env.envs.utils import Distribution, block_mean, coordinates_in_size
from drone_grid_env.envs.world.base import BaseWorld, BaseWorldConfig
...
class SimWorldConfig(BaseWorldConfig):
    from_file: Path | None = None
    ...
    objects_in_patches_position: Distribution = field(default_factory=lambda: Distribution("multivariate_normal", {"cov": [[5, 8], [8, 20]]}))
...
class SimWorld(BaseWorld[SimWorldConfig]):
    def __init__(
        self,
        rng: np.random.Generator = np.random.default_rng(),
        config: SimWorldConfig = SimWorldConfig(),
    ) -> None:
    ...
    def create_observation(self, coordinate_local: NDArray[np.uint16], fov: tuple[int, int] | NDArray[np.uint16]) -> NDArray[np.float32]:
        pad_rows, pad_columns = fov[0] // 2, fov[1] // 2

        observation_padded = np.pad(self.object_map, ((pad_rows, pad_rows), (pad_columns, pad_columns)), mode="constant", constant_values=False)
        obs_gt = observation_padded[coordinate_local[0] : coordinate_local[0] + fov[0], coordinate_local[1] : coordinate_local[1] + fov[1]]

        obs_coordinates = self.get_coordinates_with_uncertainty(obs_gt, self.config.observation_uncertainty)  # type: ignore[attr-defined]
    ...
    def prior_knowledge(self) -> NDArray[np.uint8]:
        if self._prior_knowledge_map is None:
            ...
            if self.config.prior_knowledge.from_file is not None:
                prior_knowledge = read_kml_coordinate_map(self.config.prior_knowledge.from_file, self.config.size, dtype=np.float32)
                prior_knowledge[prior_knowledge < 0.05] = 0.0  # Confidence threshold
    ...
    def create_object_map(self) -> NDArray[np.bool_]:
        field_size = np.array(self.config.size)
        field = np.zeros(field_size, dtype=np.bool_)

        if self.config.from_file is not None:  # type: ignore[attr-defined]
            object_coordinates = np.loadtxt(self.config.from_file, delimiter=" ", dtype=np.uint16)  # type: ignore[attr-defined]
            field[object_coordinates[:, 0], object_coordinates[:, 1]] = True
            return field

        n_objects = max(1, int(self.config.n_objects.get_dist_value(self.rng).round(0)))  # type: ignore[attr-defined]
        n_patches = max(1, int(self.config.n_patches.get_dist_value(self.rng).round(0)))  # type: ignore[attr-defined]

        ## Step 1: Get all cells (x,y location) in a lit
        xx, yy = np.meshgrid(range(field_size[0]), range(field_size[1]))
        cells = np.array([xx, yy]).T.reshape(np.prod(field_size), 2)

        distributions: list[Distribution] = []
        if self.config.objects_in_patches_position.dist_type == "multivariate_normal":  # type: ignore[attr-defined]
            probs = np.zeros(np.prod(field_size))

            ## Step 2: Calculate the probability per cell using the distributions
            for i in range(n_patches):
                distribution = deepcopy(self.config.objects_in_patches_position)  # type: ignore[attr-defined]
                distribution.set_value("mean", self.rng.integers((0, 0), self.config.size, size=2))
        ...
        elif self.config.objects_in_patches_position.dist_type == "uniform":  # type: ignore[attr-defined]
            distribution = deepcopy(self.config.objects_in_patches_position)  # type: ignore[attr-defined]
            distribution.set_value("low", (0, 0))
            distribution.set_value("high", field_size)

            distributions.append(distribution)
            probs = np.ones(np.prod(field_size)) / np.prod(field_size)

        else:
            dt = self.config.objects_in_patches_position.dist_type  # type: ignore[attr-defined]
        ...
        # plt.figure()
        # plt.imshow(probs.reshape(self.config.size[0], self.config.size[1]), origin="lower")
        # plt.show()

        ## Step 3: Sample N cells using the probability distribution
        max_n_objects = np.prod(field_size) - np.count_nonzero(self.start_landing_map)
    ...
    @classmethod
    def from_config_file(cls: type[SimWorld], config_file: Path, rng: np.random.Generator) -> SimWorld:
        with config_file.open("r") as cs:
            try:
                ...
                if prior_knowledge_file := world_config["prior_knowledge"]["from_file"]:
                    prior_knowledge_file = Path(prior_knowledge_file)
                ...
                config = SimWorldConfig(
                    from_file=from_file,
                    size=size,
                    start_landing_zones=start_landing_zones,
                    n_patches=n_patches,
                    n_objects=n_objects,
